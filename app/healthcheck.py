...
So this checks the contract itself, read-only:
  ...
  5. Every `agent.*` table exists (i.e. all five migrations were applied).
...
def check_agent_tables() -> bool:
    """Every agent.* table exists - i.e. all five migrations were applied."""
    from app.db.client import get_supabase
    ...
    if missing:
        ...
        _info("Apply every file in supabase/migrations/ in order (0001 -> 0005).")
        return False
